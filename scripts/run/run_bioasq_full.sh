...
echo "Dataset: MIRAGE/rawdata/bioasq (618 Yes/No questions)"
...
nohup python run.py \
  ...
  --chunks_limit 50000 \
  > bioasq_full_$(date +%Y%m%d_%H%M%S).log 2>&1 &

PID=$!
echo "============================================"
echo "Job started in background"
echo "Process ID: $PID"
echo "Monitor with: tail -f bioasq_full_*.log"
echo "Or: watch -n 2 'tail -30 bioasq_full_*.log'"
echo "============================================"
